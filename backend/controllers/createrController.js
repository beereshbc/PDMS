...
export const uploadAndDynamicParsePD = async (req, res) => {
  ...
  try {
    ...
    console.log("Python parser path:", scriptPath);
    ...
    if (!fs.existsSync(scriptPath)) {
      ...
      return res.status(500).json({
        success: false,
        message: "Dynamic parser missing",
        path: scriptPath,
      });
    }
...
const handleParsing = async (req, res, isDynamic) => {
  if (!req.file) {
    return res
      .status(400)
  ...
  try {
    ...
    if (!result.success) {
      return res.status(400).json({
        success: false,
        message: result.error || "Parser returned an error state.",
      });
    }
